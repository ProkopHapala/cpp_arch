...
class Vec2TYPE{
	...
	inline double cross ( const VEC& a ){ return x*a.y - y*a.x; }; 
	...
	inline void fromAngle_taylor2( double phi ){	sincos_taylor2<TYPE>( phi, x, y );	}

	inline void rotate( double phi ){
		double ax = cos( phi );   		  double ay = sin( phi );
		double x_ = a.x*b.x - a.y*b.y;            y = a.y*b.x + a.x*b.y;     x=x_;
	}

	inline void rotate_taylor2( double phi ){
		double ax,ay;    sincos_taylor2<TYPE>( phi, ax, ay );
		double x_ = a.x*b.x - a.y*b.y;            y = a.y*b.x + a.x*b.y;     x=x_;
	}
