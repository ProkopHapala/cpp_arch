...
class RigidBody2D : public PointBody2D {
	...
	inline void setAngle( double phi_ ){
		phi = phi_;
		rot.fromAngle( phi_ ); 
	}

	inline void sinc_rot(){
		rot.x = cos( phi );
		rot.y = sin( phi );
	}

	inline bool check_rot(){
		double r2 = rot.norm2();
		return ( fabs( r2 - 1.0d ) > ROT_NORM2_PREC );
	}
...
class SpringConstrain2D{
	...
	void draw(){
		//Vec2d gp1; gp1.set_mul_cmplx( b1->rot, p1 ); gp1.add( b1->pos );
		//Vec2d gp2; gp2.set_mul_cmplx( b2->rot, p2 ); gp2.add( b2->pos );
		Vec2d gp1; gp1.set_mul_cmplx( p1, b1->rot ); gp1.add( b1->pos );
		Vec2d gp2; gp2.set_mul_cmplx( p2, b2->rot ); gp2.add( b2->pos );
		drawLine( gp1, gp2 );
		//glBegin   (GL_LINES);	          	     
		//	glVertex3f( gp1.x, gp1.y, 0 ); glVertex3f( gp2.x, gp2.y, 0 );
		//glEnd();
