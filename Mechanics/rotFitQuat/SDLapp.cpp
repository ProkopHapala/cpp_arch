...
const float	VIEW_ZOOM_DEFAULT  = 2.0f;
...
void update( ){
	...
	printf( " i %i  |F|  %f  \n", optT->optimizer->stepsDone, sqrt( optT->optimizer->getFsqSum( ) )  );
...
void userDraw(){
	...
	glColor3f(0.0,0.0,0.0);		for (int i=0; i<npoints; i++){	drawLine( *(Vec3d*)&optT->optimizer->pos[0], optT->Tpoints[i] );		}
