...
void draw(){
	...
	double x = i2x( mouseX );
	double y = i2y( mouseY );
	double f = func1( x, y, dfdx, dfdy );
	SDL_SetRenderDrawColor(render, 255, 0, 0, 255);

	double dsc = 0.5;
	SDL_RenderDrawLine    ( render, x2i( x ), y2i( y ), x2i( x - dfdx*dsc ), y2i( y - dfdy*dsc ) ); 
	//printf( " %f %f %f %f \n", x, y, dfdx, dfdy );
...
void setup(){
	...
	func1 = &radial_1; 
	setPixelsFunction( tempSurf, 0, 0, tempSurf->w,    tempSurf->h, &potential );
