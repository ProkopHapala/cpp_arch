...
void quit(){ printf(" quit()\n" ); frameCount=2000000; SDL_Quit(); exit(1);};
...
void draw_circle( const Vec2d& pos, double r ){ 
	Vec2d a; a.set( 0, r );
	Vec2d da; da.set( 0.9807852804, 0.19509032201 );
	glBegin( GL_LINE_LOOP );
	for (int i=0; i<32; i++){
		glVertex3f( (float) pos.x+a.x, (float) pos.y+a.y, 0 );
		a.set_mulcomplex( a, da );
	}
	glEnd();
};
