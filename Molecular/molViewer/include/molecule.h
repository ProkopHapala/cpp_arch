...
class Molecule{
	...
	int drawAtom( int i, int nsphere, float atomscale, Uint32 color ){
		...
		int nvert = drawSphere_oct( nsphere, atomscale*radii[types[i]], xyzs[i] );
		return nvert;
	}
	...
	int makeViewCPK ( int nsphere, int nstick, float atomscale, float bondwidth ){
		...
		glNewList( viewlist , GL_COMPILE );
			...
			for( int i=0; i<natoms; i++ ){	nvert+= drawAtom( i, nsphere, atomscale, colors[ types[i] ] );	}
	...
	int rayPickAtom_points( const Vec3d& ray0, const Vec3d& hRay ){
		double r2min = INFINITY; int imin = -1;
		for (int i=0; i<natoms; i++){
			double t;
			double r2 = rayPointDistance2( ray0, hRay, xyzs[i], t );
			if(r2<r2min){ r2min=r2; imin=i; }
		}
		return imin;
	}

	int rayPickAtom_sphere( const Vec3d& ray0, const Vec3d& hRay ){
		double tmin = INFINITY; int imin = -1;
		for (int i=0; i<natoms; i++){
			double t = raySphere( ray0, hRay, radii[types[i]], xyzs[i] );
			if( t<tmin ){ 
