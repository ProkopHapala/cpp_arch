...
class RigidBody : public PointBody {
	...
	inline void applyForce( const Vec3d& dforce, const Vec3d& gdpos ){
		torq  .add_cross( gdpos, dforce );
		force += dforce; 
	};

	inline void applyAnchor( double k, const Vec3d& lpos, const Vec3d& gpos0 ){
		Vec3d sforce, gdpos;
		rotMat.dot_to(  lpos, gdpos   );
		sforce.set   (( gdpos + pos - gpos0 )*(-k) );
		applyForce   (  sforce, gdpos );
...
class SpringConstrain{
	...
	void apply(){
		...
		b1->applyForce ( gdp*( k), gp1 );
		b2->applyForce ( gdp*(-k), gp2 );
	}
