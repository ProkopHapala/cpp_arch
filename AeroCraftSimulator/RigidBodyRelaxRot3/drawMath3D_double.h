...
void drawPointCross( const Vec3d& vec, double dl ){
	glDisable (GL_LIGHTING);
	glBegin   (GL_LINES);	          	     
		glVertex3d( vec.x-dl, vec.y   , vec.z    ); glVertex3d( vec.x+dl, vec.y   , vec.z    );
		glVertex3d( vec.x   , vec.y-dl, vec.z    ); glVertex3d( vec.x   , vec.y+dl, vec.z    );
		glVertex3d( vec.x   , vec.y   , vec.z-dl ); glVertex3d( vec.x   , vec.y   , vec.z+dl );
	glEnd();
}
...
int shapePointsCenter( int n, Vec3d* apos, const Vec3d& center, double pointsize ){
	int ilist=glGenLists(1);
	glNewList( ilist, GL_COMPILE );
	for(int i=0;  i<n; i++){
		drawLine( center, apos[i] );
		drawPointCross( apos[i], pointsize );
	};
	glEndList();
	return( ilist );
}
