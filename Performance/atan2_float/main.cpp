...
const float IZOOM = 1.0d/ZOOM;

float i2x( int ix ){ return (ix - IX0)*IZOOM;  };
float i2y( int iy ){ return (iy - IY0)*IZOOM;  };

int x2i( float  x ){ return x*ZOOM + IX0;     };
int y2i( float  y ){ return y*ZOOM + IY0;     };
...
float * xs = new float[nsamples];
float * ys = new float[nsamples];
...
void draw(){
	...
	for (int i=0; i<ntimes; i++  ){
		int ii   = i & 0x3FF;		float x = xs[ii];		float y = ys[ii];
		sum += x + y;
	}  
	t2 =  rdtsc();
